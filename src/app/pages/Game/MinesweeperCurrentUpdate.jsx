...
export default function Minesweeper() {
    ...
    const [SelectedGameMode, setSelectedGameMode] = useState('Test');
    ...
    useEffect(() => {
        ...
        setTime(-1);
        setHasWon(0);
        // generateGameBoardBombAndReveal();
    }, [GameMode, Refresh]);

    const generateGameBoardBombAndReveal = (firstRow, firstCol) => {
        console.log('generateGameBoardBombAndReveal');
        ...
        while (count < GameMode.bombNumber) {
            ...
            if (NewGameBoard[row][col].value !== 9 && !(Math.abs(row - firstRow) <= 1 && Math.abs(col - firstCol) <= 1)) {
                NewGameBoard[row][col].value = 9;
                count++;
            }
        }

        setGameBoard(p => NewGameBoard);
        console.log('generateGameBoardBombAndReveal Success');
    };
                            ...
                            <tr key={index_row}>
                                {[...Array(GameMode.colCount)].map((_, index_col) => (
                                    <td key={index_col}
                                        ...
                                        onClick={() => Time === -1 ? generateGameBoardBombAndReveal(index_row, index_col) : revealCell(index_row, index_col)}
